...
def statement(tok, linenum):
  ...
  if tok.type == "PRINT":
    ...
    statements.append(GotoStatement(linenum))
    tok = goto(tok)
  elif tok.type == "GOSUB":
    statements.append(GosubStatement(linenum))
    tok = gosub(tok)
  elif tok.type == "IF":
    statements.append(IfStatement(linenum))
    tok = myif(tok, linenum)
  elif tok.type == "END":
    statements.append(EndStatement(linenum))
    tok = lexer.token()
  elif tok.type == "RETURN":
    statements.append(ReturnStatement(linenum))
    tok = lexer.token()
  elif tok.type == "REM":
...
def relop(tok):
  if tok.type == "LESS":
    ...
    if tok.type == "GREATER" or tok.type == "EQUALS":
      tok = lexer.token() # grab the extra token since the token we just "consumed" (i.e. checked) was part of the relop
  elif tok.type == "GREATER":
    tok = lexer.token()
    if tok.type == "LESS" or tok.type == "EQUALS":
      tok = lexer.token() # grab the extra token since the token we just "consumed" (i.e. checked) was part of the relop
  elif tok.type == "EQUALS":
    tok = lexer.token() # nothing to check for here so we need to grab an exxtra token in order to return an "extra" token
  else:
...
def var_list(tok):
  # look for a var first and ERROR out if we don't have one
  if tok.type != "VAR":
    tokerror(tok, "VAR")
  tok = lexer.token()
...
for stmt in statements:
  print(f"Executing {stmt} at line # {stmt.linenumber}")
  stmt.execute()
